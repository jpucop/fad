...
def load_base_defs() -> Dict[str, Dict[str, Any]]:
  ...
  for key, props in raw.items():
    if not isinstance(props, dict):
      ...
      continue
    if "[" in key:
      cls, parent = key.strip().split("[")
      parent = parent.rstrip("]")
      defs[cls] = {"__base__": parent, "__props__": list(props.keys()), "__total_props__": set(props.keys())}
    else:
      defs[key] = {"__base__": None, "__props__": list(props.keys()), "__total_props__": set(props.keys())}
    # Compute total properties
    p = defs[key]["__base__"]
    while p and p in defs:
      defs[key]["__total_props__"].update(defs[p]["__props__"])
      p = defs[p]["__base__"]
...
def generate_models(schema_dir: Path, model_output_dir: Path, base_defs: dict) -> tuple[set, Dict[str, Any]]:
  ...
  for base_name, base_info in base_defs.items():
    ...
    base_class = f"backend.app.models.{parent}.{parent.title()}" if parent else "pydantic.BaseModel"
    output_path = model_output_dir / f"{base_name}.py"
    generate(
      input_=json.dumps(schema, indent=2),
      input_file_type=InputFileType.JsonSchema,
      output=output_path,
      class_name=base_name.title(),
  ...
  for name, schema in schemas.items():
    ...
    # Find the most specific base class
    ...
    for base_name, base_info in sorted(base_defs.items(), key=lambda x: len(x[1]["__total_props__"]), reverse=True):
      if base_info["__total_props__"].issubset(props):
        base_class = f"backend.app.models.{base_name}.{base_name.title()}"
...
def load_pydantic_model(model_name: str) -> Type[BaseModel]:
  ...
  try:
    ...
    model_class = getattr(module, model_name.title())
