...
def infer_property_type(value: Any, prop_name: str, file_name: str) -> Dict[str, Any]:
  ...
  if isinstance(value, dict):
    return {"type": "object"}  # Could recurse for nested, left flat for now
...
def generate_models(all_files: Dict[str, List[Tuple[str, dict]]], named: Set[str], namedesc: Set[str]):
  for category, entries in all_files.items():
    for name, data in entries:
      ...
      temp_schema_path.write_text(json.dumps(schema, indent=2), encoding="utf-8")
      cmd = [
        "datamodel-codegen", "--input", str(temp_schema_path), "--input-file-type", "jsonschema",
        "--output", str(output_path), "--use-default", "--disable-timestamp",
        "--strip-default-none", "--class-name", f"{name.title().replace('_','')}Model"
      ]
