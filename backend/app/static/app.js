...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
    ...
    function bindInputValue2(el, value) {
      if (isRadio2(el)) {
        ...
        if (window.fromModel) {
          if (typeof value === "boolean") {
            el.checked = safeParseBoolean2(el.value) === value;
          } else {
            el.checked = checkedAttrLooseCompare3(el.value, value);
          }
        }
      } else if (isCheckbox2(el)) {
        if (Number.isInteger(value)) {
          el.value = value;
        } else if (!Array.isArray(value) && typeof value !== "boolean" && ![null, void 0].includes(value)) {
          el.value = String(value);
    ...
    function safeParseBoolean2(rawValue) {
      if ([1, "1", "true", "on", "yes", true].includes(rawValue)) {
        return true;
      }
      if ([0, "0", "false", "off", "no", false].includes(rawValue)) {
    ...
    function isCheckbox2(el) {
      return el.type === "checkbox" || el.localName === "ui-checkbox" || el.localName === "ui-switch";
    }
    function isRadio2(el) {
      return el.type === "radio" || el.localName === "ui-radio";
    }
    ...
    function createGetter2(isReadonly = false, shallow = false) {
      return function get32(target, key, receiver) {
        ...
        if (isObject2(res)) {
          return isReadonly ? readonly2(res) : reactive22(res);
        }
        return res;
      };
    }
    ...
    magic2("watch", (el, { evaluateLater: evaluateLater22, cleanup: cleanup22 }) => (key, callback) => {
    ...
    magic2("id", (el, { cleanup: cleanup22 }) => (name, key = null) => {
      let cacheKey = `${name}${key ? `-${key}` : ""}`;
      return cacheIdByNameOnElement2(el, cacheKey, cleanup22, () => {
    ...
    function cacheIdByNameOnElement2(el, cacheKey, cleanup22, callback) {
      if (!el._x_id)
        el._x_id = {};
      if (el._x_id[cacheKey])
        return el._x_id[cacheKey];
    ...
    directive2("effect", skipDuringClone2((el, { expression }, { effect: effect32 }) => {
      effect32(evaluateLater2(el, expression));
    }));
    function on2(el, event, modifiers, callback) {
      ...
      if (isKeyEvent2(event) || isClickEvent2(event)) {
        handler42 = wrapHandler(handler42, (next, e) => {
    ...
    directive2("model", (el, { modifiers, expression }, { effect: effect32, cleanup: cleanup22 }) => {
      ...
      if (modifiers.includes("fill")) {
        if ([void 0, null, ""].includes(getValue()) || isCheckbox2(el) && Array.isArray(getValue()) || el.tagName.toLowerCase() === "select" && el.multiple) {
          setValue(
            getInputValue2(el, modifiers, { target: el }, getValue())
          );
        }
      }
      if (!el._x_removeModelListeners)
      ...
      if (el.form) {
        let removeResetListener = on2(el.form, "reset", [], (e) => {
          nextTick2(() => el._x_model && el._x_model.set(getInputValue2(el, modifiers, { target: el }, getValue())));
        });
      ...
      return mutateDom2(() => {
        if (event instanceof CustomEvent && event.detail !== void 0)
          ...
          if (Array.isArray(currentValue)) {
            let newValue = null;
            if (modifiers.includes("number")) {
              newValue = safeParseNumber2(event.target.value);
            } else if (modifiers.includes("boolean")) {
              newValue = safeParseBoolean2(event.target.value);
            } else {
              newValue = event.target.value;
            }
            return event.target.checked ? currentValue.includes(newValue) ? currentValue : currentValue.concat([newValue]) : currentValue.filter((el2) => !checkedAttrLooseCompare22(el2, newValue));
          } else {
            return event.target.checked;
          }
        } else if (el.tagName.toLowerCase() === "select" && el.multiple) {
          if (modifiers.includes("number")) {
            ...
            return Array.from(event.target.selectedOptions).map((option) => {
              let rawValue = option.value || option.text;
              return safeParseBoolean2(rawValue);
            });
          }
          return Array.from(event.target.selectedOptions).map((option) => {
            return option.value || option.text;
          ...
          if (modifiers.includes("number")) {
            return safeParseNumber2(newValue);
          } else if (modifiers.includes("boolean")) {
            return safeParseBoolean2(newValue);
          } else if (modifiers.includes("trim")) {
            return newValue.trim();
          } else {
            return newValue;
    ...
    var handler22 = (el, { value, modifiers, expression, original }, { effect: effect32, cleanup: cleanup22 }) => {
      if (!value) {
        let bindingProviders = {};
    ...
    function warnMissingPluginDirective2(name, directiveName, slug) {
      directive2(directiveName, (el) => warn2(`You can't use [x-${directiveName}] without first installing the "${name}" plugin here: https://alpinejs.dev/plugins/${slug}`, el));
    }
    ...
    var src_default2 = alpine_default2;
